...
class QLearning:
    def __init__(self, actions, agent_indicator=10):
        ...
        self.alpha = 0.01
        #hyperparmaeter tuning (original = 0.9) 
        self.gamma = 0.95
        #(0.2)
        self.epsilon = 0.8
...
for ep in range(20000):
    ...
    while not done:
        ...
        if reward == 0:  # 현재 보상이 0인 경우에는 살았다는 작은 페널티 부여
            reward = -0.001
        if done and reward > 0:  # 목표에 도달했을 경우 추가 보상 부여
            reward += 10
    ...
    #epsilon decay적용 
    agent.epsilon = max(0.2, agent.epsilon * 0.99)
    if (ep+1) % 20 == 0:
